...
def get_server_actions_keyboard(server_id: int) -> InlineKeyboardBuilder:
    ...
    builder.row(
        InlineKeyboardButton(text="🔙 Назад к списку", callback_data="admin_servers_list"),
        InlineKeyboardButton(text="🏠 В меню", callback_data="admin_menu"),
    )
    return builder
...
def get_plan_actions_keyboard(plan_id: int) -> InlineKeyboardBuilder:
    ...
    builder = InlineKeyboardBuilder()
    builder.row(
        InlineKeyboardButton(text="✏️ Редактировать", callback_data=f"admin_edit_plan_{plan_id}"),
    )
    builder.row(
        InlineKeyboardButton(text="🗑 Удалить", callback_data=f"admin_delete_plan_{plan_id}"),
    )
    builder.row(
        InlineKeyboardButton(text="🔙 Назад к списку", callback_data="admin_plans_list"),
        InlineKeyboardButton(text="🏠 В меню", callback_data="admin_menu"),
    )
    return builder
...
def get_subscriptions_list_keyboard(subscriptions: list, page: int = 0) -> InlineKeyboardBuilder:
    ...
    if page > 0:
        builder.row(
            InlineKeyboardButton(text="⬅️ Назад", callback_data=f"admin_subs_page_{page - 1}")
        )
    if len(subscriptions) == 10:
        builder.row(
            InlineKeyboardButton(text="Вперёд ➡️", callback_data=f"admin_subs_page_{page + 1}")
        )
...
def get_subscription_actions_keyboard(subscription_id: int, user_id: int) -> InlineKeyboardBuilder:
    ...
    builder = InlineKeyboardBuilder()
    builder.row(
        InlineKeyboardButton(text="✏️ Продлить", callback_data=f"admin_extend_sub_{subscription_id}"),
    )
    builder.row(
        InlineKeyboardButton(text="🗑 Удалить подписку", callback_data=f"admin_delete_sub_{subscription_id}"),
    )
    builder.row(
        InlineKeyboardButton(text="👤 К пользователю", callback_data=f"admin_user_{user_id}"),
    )
    builder.row(
        InlineKeyboardButton(text="🔙 Назад к списку", callback_data="admin_subscriptions_list"),
        InlineKeyboardButton(text="🏠 В меню", callback_data="admin_menu"),
    )
    return builder
